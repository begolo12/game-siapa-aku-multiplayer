  ...
  return (
    <header className="bg-[#2b241c]/90 backdrop-blur-md border-b border-slate-900 sticky top-0 z-40">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between gap-2 h-14 items-center min-w-0">
          {/* Logo / Title */}
          ...
          {/* User badge only */}
          {currentUser && (
            <div className="flex shrink-0 items-center gap-2 bg-slate-900/80 border border-slate-800/80 px-2 sm:px-3 py-1 sm:py-1.5 rounded-xl hover:border-slate-700 transition-colors">
              <div className="flex min-w-0 flex-col items-end">
                <div className="flex items-center gap-1">
                  {currentUser.isAdmin && <Shield className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-red-400" />}
                  <span className="max-w-24 truncate text-xs sm:max-w-none sm:text-sm font-semibold text-slate-200">{currentUser.username}</span>
                </div>
